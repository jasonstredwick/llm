 ...
 * AsyncResult (blocking) or CoroResult (coroutine).
 *
 ...
 *
 * For the coroutine path (CoroResult / CallCoro), a SyncAwaiter stores
...
struct ResultSync {
    ...
    // Reset for retry — called by AsyncResult or CoroResult after deserialization failure.
    void Reset() {
        std::lock_guard lock(mtx);
        ready = false;
        succeeded = false;
 ...
 *
 * Used by CoroResult's coroutine body (CallCoro) to suspend at the
