...
AsyncResult<Message> Client::CallAsync(const Request& r,
    ...
    return AsyncResult<Message>{orchestrator, ticket, &Deserialize, std::move(sync)};
}
...
CoroAsyncResult<Message> Client::CallCoro(const Request& r,
                                     const AttemptPolicy& call_policy) const {
