...
// Note: The model name in the URL will depend on the request's model field.
// We'll construct it in GenUrl.
...
std::vector<std::byte> Serialize(const Request& r) {
            ...
            for (size_t j = 0; j < c.parts.size(); ++j) {
                ...
                std::visit([&sb](auto const& part) {
                    ...
                    if constexpr (std::is_same_v<T, Content::ContentPart::TextPart>) {
                        ...
                        sb.append_colon();
                        sb.start_object();
                        sb.append_key_value("language", jai::llm::to_string_view(part.language));
                        sb.append_comma();
                        ...
                        sb.append_colon();
                        sb.start_object();
                        sb.append_key_value("outcome", jai::llm::to_string_view(part.outcome));
                        sb.append_comma();
    ...
    if (r.config) {
        ...
        sb.start_object();
        bool first = true;
        if (r.config->temperature) {
            if (!first) sb.append_comma();
            sb.append_key_value("temperature", *r.config->temperature);
            first = false;
        }
        if (r.config->top_p) {
            if (!first) sb.append_comma();
            sb.append_key_value("topP", *r.config->top_p);
            first = false;
        }
        if (r.config->top_k) {
            if (!first) sb.append_comma();
            sb.append_key_value("topK", *r.config->top_k);
            first = false;
        }
        if (r.config->max_output_tokens) {
            if (!first) sb.append_comma();
            sb.append_key_value("maxOutputTokens", *r.config->max_output_tokens);
            first = false;
        }
        if (!r.config->stop_sequences.empty()) {
            if (!first) sb.append_comma();
        ...
        if (r.config->response_mime_type) {
            if (!first) sb.append_comma();
            sb.append_key_value("responseMimeType", jai::llm::to_string_view(*r.config->response_mime_type));
            first = false;
        }
        if (r.config->response_schema) {
            if (!first) sb.append_comma();
            sb.escape_and_append_with_quotes("responseSchema");
            sb.append_colon();
            sb.append_raw(*r.config->response_schema); // Verbatim JSON schema
            first = false;
        }
        if (r.config->thinking_config) {
            if (!first) sb.append_comma();
            sb.escape_and_append_with_quotes("thinkingConfig");
            sb.append_colon();
            sb.start_object();
            sb.append_key_value("includeThoughts", r.config->thinking_config->include_thoughts);
            if (r.config->thinking_config->thinking_budget > 0) {
                sb.append_comma();
                sb.append_key_value("thinkingBudget", r.config->thinking_config->thinking_budget);
            }
            sb.end_object();
            first = false;
        }
    ...
    if (r.tool_config) {
        ...
        sb.append_colon();
        sb.start_object();
        sb.append_key_value("mode", jai::llm::to_string_view(r.tool_config->function_calling_config->mode));
